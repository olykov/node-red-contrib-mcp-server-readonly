...
//     requiredClaim, requiredValue,            // the server-wide claim gate (what the user may do)
//     requiredScope,                           // the server-wide scope gate (what the client may do)
...
async function handleRpc(body, claims, deps) {
    ...
    const isToolResult = value => value && typeof value === 'object' && !Array.isArray(value) &&
        ['content', 'structuredContent', '_meta', 'isError'].some(key => Object.prototype.hasOwnProperty.call(value, key));
    ...
    // Two axes, ANDed: the claim says what the user may do, the scope what the client was
    // authorized to do on their behalf (see createToolGate).
    ...
    if (method === 'resources/list') {
        if (!allowed) return respond({ resources: [] });
        return respond({ resources: Object.values(resources).map(({ uri, name, description, mimeType }) =>
            ({ uri, name, description, mimeType })) });
    }

    if (method === 'resources/read') {
        if (!allowed) return rpcErr(-32001, 'Access denied: your account lacks the required permission to use this server.');
        const resource = resources[params.uri];
        if (!resource) return rpcErr(-32602, 'Unknown resource: ' + String(params.uri));
        return respond({ contents: [{ uri: resource.uri, mimeType: resource.mimeType, text: resource.text, _meta: resource._meta }] });
    }
        ...
        if (entry) {
            ...
            try {
                ...
                return isToolResult(result)
                    ? respond(result)
                    : (Array.isArray(result) ? respond({ content: result }) : toolOk(result));
            } catch (e) {
