...
async function resolveFlowResult(ctx, mcpServer, result) {
    ...
    if (!hasOpenAIFormCapability(mcpServer)) {
        const summary = capabilitySummary(mcpServer);
        return textResult('OpenAI form elicitation is not advertised by this MCP client connection' +
